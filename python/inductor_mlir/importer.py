from ._C.inductor_mlir.ir_builder import IRBuilder
from .monkey_patch.context_manager import patch
import inductor_mlir._C.inductor_mlir.ir_builder as inductor_mlir
...
class InductorImporter:
    ...
    def into_graph_run(self):

        def run_node_fn(origin_fn):

            def inner(*args, **kw_args):
                return self.graph_lowering_run_node(origin_fn, *args,
                                                    **kw_args)

            return inner

        def placeholder_fn(origin_fn):

            def inner(*args, **kw_args):
                return self.graph_lowering_placeholder(origin_fn, *args,
                                                       **kw_args)

            return inner

        with patch(torch._inductor.graph.GraphLowering, 'run_node',
                   run_node_fn), patch(torch._inductor.graph.GraphLowering,
                                       'placeholder', placeholder_fn):
            yield
        self.finish_graph_run()

    def graph_lowering_placeholder(self, origin_fn, graph, target, args,
                                   kwargs):
        self.graph_input_names.append(target)
        res = origin_fn(graph, target, args, kwargs)
        return res

    def graph_lowering_run_node(self, origin_fn, graph, n: torch.fx.Node):
        res = origin_fn(graph, n)
        if isinstance(res, _inductor.ir.TensorBox):
            if isinstance(res.data, _inductor.ir.StorageBox):
                storage = res.data
                if isinstance(storage.data, _inductor.ir.Buffer):
                    buffer = storage.data
                    assert id(buffer) not in self.buffer_to_tensor_map
                    self.buffer_to_tensor_map[id(buffer)] = res
                    if isinstance(getattr(buffer, 'data', None),
                                  _inductor.ir.Loops):
                        loop = buffer.data
                        assert id(loop) not in self.loop_to_buffer_map and id(
                            loop) not in self.loop_to_buffer_map
                        self.loop_to_buffer_map[id(loop)] = buffer
                if isinstance(res.data, _inductor.ir.Loops):
                    loop = res.data
                    assert id(loop) not in self.loop_to_buffer_map and id(
                        loop) not in self.loop_to_tensor_map
                    self.loop_to_tensor_map[id(loop)] = res
        return res

    @classmethod
    def register_wrapper(cls, wrapper, node):
        assert node not in cls.wrapper_registry
        cls.wrapper_registry[node] = wrapper

    @classmethod
    def get_wrapper(cls, node):
        ty = type(node)
        wrapper = cls.wrapper_registry.get(ty, None)
        assert wrapper is not None, ty
        return wrapper
...
def register_wrapper(node_cls):

    def func(wrapper):
        InductorImporter.register_wrapper(wrapper, node_cls)
        return wrapper
